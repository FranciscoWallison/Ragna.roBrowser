...
import TextEncoding from "../Vendors/text-encoding.js";
import fengari from "../Vendors/fengari-web.js";
...
import RandomOption from "./Items/ItemRandomOptionTable.js";
import SKID from "./Skills/SkillConst.js";
import SkillDescription from "./Skills/SkillDescription.js";
...
import QuestInfo from "./QuestTable.js";
 ...
 */
DB.INTERFACE_PATH =
	"data/texture/\xc0\xaf\xc0\xfa\xc0\xce\xc5\xcd\xc6\xe4\xc0\xcc\xbd\xba/";
...
DB.init = function init() {
	...
	if (Configs.get("loadLua")) {
		...
		loadLuaTable(
			[
				DB.LUA_PATH + "datainfo/accessoryid.lub",
				DB.LUA_PATH + "datainfo/accname.lub",
			],
			"AccNameTable",
			function (json) {
				HatTable = json;
		...
		loadLuaTable(
			[
				DB.LUA_PATH + "datainfo/spriterobeid.lub",
				DB.LUA_PATH + "datainfo/spriterobename.lub",
			],
			"RobeNameTable",
			function (json) {
				RobeTable = json;
		...
		loadLuaTable(
			[
				DB.LUA_PATH + "datainfo/npcidentity.lub",
				DB.LUA_PATH + "datainfo/jobname.lub",
			],
			"JobNameTable",
			function (json) {
				MonsterTable = json;
		...
		loadLuaTable(
			[
				DB.LUA_PATH + "datainfo/enumvar.lub",
				DB.LUA_PATH + "datainfo/addrandomoptionnametable.lub",
			],
			"NameTable_VAR",
			function (json) {
				RandomOption = json;
		...
		loadLuaTable(
			[
				...
				DB.LUA_PATH + "skillinfoz/skilldescript.lub",
			],
			"SKILL_DESCRIPT",
			function (json) {
		...
		loadLaphineSysFile(
			DB.LUA_PATH + "datainfo/lapineddukddakbox.lub",
			function (laphinesys_list) {
				LaphineSysTable = laphinesys_list;
			},
			onLoad()
		);
	} else {
		loadTable(
			...
			function (index, key, val) {
				(
					ItemTable[key] || (ItemTable[key] = {})
				).unidentifiedDisplayName = val.replace(/_/g, " ");
			},
			onLoad()
		);
		loadTable(
		...
		loadTable(
			...
			function (index, key, val) {
				(
					ItemTable[key] || (ItemTable[key] = {})
				).unidentifiedDescriptionName = val.split("\n");
			},
			onLoad()
		);
		loadTable(
			...
			function (index, key, val) {
				(
					ItemTable[key] || (ItemTable[key] = {})
				).identifiedDisplayName = val.replace(/_/g, " ");
			},
			onLoad()
		);
		loadTable(
		...
		loadTable(
			...
			function (index, key, val) {
				(
					ItemTable[key] || (ItemTable[key] = {})
				).identifiedDescriptionName = val.split("\n");
			},
			onLoad()
		);
		loadTable(
	...
	loadTable(
		...
		2,
		function (index, key, val) {
			(ItemTable[key] || (ItemTable[key] = {})).processitemlist =
				val.split("\n");
		},
		onLoad()
	);
	...
	Client.loadFile(
		...
		async function (lua) {
			...
			try {
				...
				if (!fengari.lua.lua_istable(fengari.L, -1)) {
					console.log(
						"[loadLapineFile] tblLapineDdukddakBox is not a table"
					);
					return;
				}
				...
				while (fengari.lua.lua_next(fengari.L, -2)) {
					...
					source.NeedRefineMin = fengari.lua.lua_tointeger(
						fengari.L,
						-1
					);
					fengari.lua.lua_pop(fengari.L, 1);
					...
					source.NeedRefineMax = fengari.lua.lua_tointeger(
						fengari.L,
						-1
					);
					fengari.lua.lua_pop(fengari.L, 1);
					...
					fengari.lua.lua_getfield(
						fengari.L,
						-1,
						"NeedSource_String"
					);
					source.NeedSource_String = fengari.lua.lua_tojsstring(
						fengari.L,
						-1
					);
					fengari.lua.lua_pop(fengari.L, 1);
					...
					while (fengari.lua.lua_next(fengari.L, -2)) {
						...
						sourceItem.name = fengari.lua.lua_tojsstring(
							fengari.L,
							-1
						);
						fengari.lua.lua_pop(fengari.L, 1);
						...
						fengari.lua.lua_gettable(fengari.L, -2);
						sourceItem.count = fengari.lua.lua_tointeger(
							fengari.L,
							-1
						);
						fengari.lua.lua_pop(fengari.L, 1);
						...
						fengari.lua.lua_gettable(fengari.L, -2);
						sourceItem.id = fengari.lua.lua_tointeger(
							fengari.L,
							-1
						);
						fengari.lua.lua_pop(fengari.L, 1);
...
function lua_remove_comments(content) {
	...
	while ((start = content.indexOf("--[[")) !== -1) {
		...
		content =
			...
			content.substring(end + 4, end.length);
	}
...
function loadLuaTable(file_list, table_name, callback, onEnd) {
	...
	Client.loadFile(id_filename, async function (data) {
		try {
	...
	function loadValueTable() {
		...
		Client.loadFile(value_table_filename, async function (data) {
			try {
				// check if is ArrayBuffer or String
				if (data instanceof ArrayBuffer) {
					data = new TextDecoder("iso-8859-1").decode(data);
	...
	function parseTable() {
		...
		while (fengari.lua.lua_next(fengari.L, -2)) {
			...
			if (fengari.lua.lua_istable(fengari.L, -1)) {
				...
				while (fengari.lua.lua_next(fengari.L, -2) != 0) {
					// skilldescription
					if (fengari.lua.lua_isstring(fengari.L, -1)) {
						value_array.push(
							fengari.lua.lua_tojsstring(fengari.L, -1) + "\n"
						);
					}
					fengari.lua.lua_pop(fengari.L, 1);
				}
...
function isPlayer(jobid) {
	return (
		jobid < 45 || (jobid >= 4001 && jobid <= 4317) || jobid == 4294967294
	);
}
...
DB.getBodyPath = function getBodyPath(id, sex, alternative) {
	...
	if (isNPC(id)) {
		return (
			"data/sprite/npc/" +
			(MonsterTable[id] || MonsterTable[46]).toLowerCase()
		);
	}
	...
	if (isHomunculus(id)) {
		return (
			"data/sprite/homun/" +
			(MonsterTable[id] || MonsterTable[1002]).toLowerCase()
		);
	}
	...
	// MONSTER
	return (
		"data/sprite/\xb8\xf3\xbd\xba\xc5\xcd/" +
		(MonsterTable[id] || MonsterTable[1001]).toLowerCase()
	);
};
...
DB.getHatPath = function getHatPath(id, sex) {
	...
	return (
		"data/sprite/\xbe\xc7\xbc\xbc\xbb\xe7\xb8\xae/" +
		SexTable[sex] +
		"/" +
		SexTable[sex] +
		HatTable[id]
...
DB.getPCAttackMotion = function getPCAttackMotion(
	job,
	sex,
	weapon,
	isDualWeapon
...
DB.getWeaponPath = function getWeaponPath(id, job, sex, leftid = false) {
	...
	if (leftid) {
		...
		// Create dualhand Id
		var right = Object.keys(WeaponType).find(
			(key) => WeaponType[key] === id
		);
		var left = Object.keys(WeaponType).find(
			(key) => WeaponType[key] === leftid
		);
		if (right && left) {
...
DB.getWeaponViewID = (function getWeaponViewIdClosure() {
	...
	return function getWeaponViewID(id) {
		...
		return id < 1150
			...
			? WeaponType.NONE
			: id < 13150
			? WeaponType.GUN_HANDGUN
			: id < 13200
			? WeaponType.GUN_RIFLE
			: id < 13300
			? WeaponType.NONE
			: id < 13350
			? WeaponType.SYURIKEN
		...
		if (!item._decoded) {
			...
			item.unidentifiedDescriptionName =
				...
				item.unidentifiedDescriptionName instanceof Array
					...
					: "";
			item.identifiedDisplayName = TextEncoding.decodeString(
				item.identifiedDisplayName
			);
			item.unidentifiedDisplayName = TextEncoding.decodeString(
...
DB.getItemName = function getItemName(item, options = {}) {
			...
			default:
				...
				for (i = 1; i <= 4; ++i) {
					...
					if (cards[card]) {
						...
						cards[card].prefixName =
							DB.getItemInfo(card).prefixName;
						cards[card].count = 0;
					}
				}
	...
	if (item.Options && showItemOptions) {
		let numOfOptions = item.Options.filter(
...
DB.getPetTalk = function getPetTalk(data) {
	...
	if (
		hungryState >= Object.keys(PetHungryState).length ||
		actionState >= Object.keys(PetMessageConst).length
	) {
		return false;
	}
	...
	if (
		...
		if (PetTalkTable[mobName][hungryText][actionText] instanceof Array) {
			rnd = parseInt(
				(Math.random() * 100) %
					PetTalkTable[mobName][hungryText][actionText].length
			);
			return TextEncoding.decodeString(
				PetTalkTable[mobName][hungryText][actionText][rnd]
			);
		}
		return TextEncoding.decodeString(
			PetTalkTable[mobName][hungryText][actionText]
