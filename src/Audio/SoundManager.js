 ...
 * All browsers seem to support .wav file (with HTML5), so don't need a flash callback here
 *
...
SoundManager.play = function play(filename, vol) {
	...
	Client.loadFile("data/wav/" + filename, function (url) {
		...
		// Initialize the sound and play it
...
SoundManager.playPosition = function playPosition(filename, srcPosition) {
	const dist = Math.floor(
		glMatrix.dist(srcPosition, Session.Entity.position)
	);
	const vol = Math.max(
		1 - Math.abs(((dist - 1) * (1 - 0.01)) / (25 - 1) + 0.01),
		0.1
	);
...
/**
 * Move sound to cache.
 * If we have a request to play the same sound again, get it back
...
function addSoundToCache(sound) {
	if (sound.filename) {
		if (!(sound.filename in _cache)) {
			_cache[sound.filename] = {};
			_cache[sound.filename].instances = [];
		}

		// Don't cache too many instances (self balancing formula based on total media players)
		if (
			_cache[sound.filename].instances.length <
			balancedMax(C_MAX_CACHED_SOUND_INSTANCES)
		) {
...
function cleanupCache(sound) {
	if (
		sound.filename &&
		sound.filename in _cache &&
		_cache[sound.filename].instances.length > 0
	) {
