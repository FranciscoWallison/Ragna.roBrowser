...
SpriteRenderer.init = function init(gl) {
	if (!_buffer) {
		...
		gl.bufferData(
			gl.ARRAY_BUFFER,
			new Float32Array([
				-0.5, +0.5, 0.0, 0.0, +0.5, +0.5, 1.0, 0.0, -0.5, -0.5, 0.0,
				1.0, +0.5, -0.5, 1.0, 1.0,
			]),
			gl.STATIC_DRAW
		);
	...
	if (!_program) {
		_program = WebGL.createShaderProgram(
			gl,
			_vertexShader,
			_fragmentShader
		);
 ...
 */
SpriteRenderer.bind3DContext = function Bind3dContext(
	gl,
	modelView,
	projection,
	fog
