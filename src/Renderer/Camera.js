		...
		if (this.quakes[i].active) {
			if (this.quakes[i].startTick <= tick) {
				if (this.quakes[i].startTick + this.quakes[i].duration > tick) {
					var step =
						(tick - this.quakes[i].startTick) /
						this.quakes[i].duration;

					this.position[0] +=
						((Math.random() * 5 - 2.5) / 10 +
							this.quakes[i].sideQuake) *
					...
					this.zoom +=
						((Math.random() * 5 - 2.5) / 10 +
							this.quakes[i].zoomQuake) *
						C_QUAKE_MULT;
					this.quakes[i].zoomQuake *= -1;

					this.angle[0] +=
						((Math.random() * 5 - 2.5) / 15 +
							this.quakes[i].latitudeQuake) *
						C_QUAKE_MULT;
...
Camera.processMouseAction = function ProcessMouseAction() {
	...
	if (KEYS.SHIFT) {
		this.angleFinal[0] +=
			((Mouse.screen.y - this.action.y) / Mouse.screen.height) * 300;
		if (DB.isIndoor(this.currentMap)) {
			this.angleFinal[0] = Math.max(
				this.angleFinal[0],
				this.MIN_ALTITUDE_INDOOR
			);
			this.angleFinal[0] = Math.min(
				this.angleFinal[0],
				this.MAX_ALTITUDE_INDOOR
			);
		} else {
	...
	// Zoom
		...
		this.zoomFinal -=
			(Mouse.screen.y - this.action.y) *
			((this.zoomStep * this.zoomStepMult) / 10);
		if (DB.isIndoor(this.currentMap)) {
			this.zoomFinal = Math.min(
				this.zoomFinal,
				Math.abs(this.altitudeRange) * this.MAX_ZOOM_INDOOR
			);
		} else {
			this.zoomFinal = Math.min(
				this.zoomFinal,
				Math.abs(this.altitudeRange) * this.MAX_ZOOM
			);
		}
		this.zoomFinal = Math.max(
			this.zoomFinal,
			Math.abs(this.altitudeRange) * this.MIN_ZOOM
		);
	}
	...
	// Rotate
		...
		this.angleFinal[1] -=
			((Mouse.screen.x - this.action.x) / Mouse.screen.width) * 720;
		...
		if (DB.isIndoor(this.currentMap)) {
			this.angleFinal[1] = Math.max(
				this.angleFinal[1],
				this.indoorRotationFrom
			);
			this.angleFinal[1] = Math.min(
				this.angleFinal[1],
				this.indoorRotationTo
			);
		} else {
			this.angleFinal[1] = Math.max(
				this.angleFinal[1],
...
Camera.updateState = function UpdateState() {
	if (this.enable1STPerson && this.zoomFinal == 0) {
		...
		this.zoomFinal <
			Math.abs(this.altitudeRange) * C_THIRDPERSON_TRESHOLD_ZOOM
	) {
...
Camera.update = function Update(tick) {
	...
	// Updimport camera from mouse movement
	...
	if (Preferences.smooth && this.state != this.states.first_person) {
		this.position[0] +=
			(-this.target.position[0] - this.position[0]) * lerp;
		this.position[1] +=
			(-this.target.position[1] - this.position[1]) * lerp;
