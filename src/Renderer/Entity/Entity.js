...
Entity.prototype.set = function Set(unit) {
	...
	for (i = 0; i < count; ++i) {
		switch (keys[i]) {
			...
			case "objecttype":
				this.objecttype =
					unit.job === 45 ? Entity.TYPE_WARP : unit.objecttype;
				require("./EntityAction").call(this);
				break;
			...
			case "PosDir":
				this.direction = [4, 3, 2, 1, 0, 7, 6, 5][
					(unit.PosDir[2] + 8) % 8
				];
			...
			case "MoveData":
				...
				this.position[2] = Altitude.getCellHeight(
					unit.MoveData[0],
					unit.MoveData[1]
				);
				this.walkTo.apply(this, [
					unit.MoveData[0],
					unit.MoveData[1],
					unit.MoveData[2],
					unit.MoveData[3],
			...
			default:
				if (
					Entity.prototype.hasOwnProperty(keys[i]) ||
					Entity.prototype.hasOwnProperty("_" + keys[i])
				) {
