 ...
 * @param mixed data
 */
MemoryItem.prototype.addEventListener = function addEventListener(
	event,
	callback
) {
	if (!(callback instanceof Function)) {
		throw new Error(
			"MemoryItem::addEventListener() - callback must be a function !"
		);
	}
	...
	switch (event.toLowerCase()) {
		...
		default:
			throw new Error(
				'MemoryItem::addEventListener() - Invalid event "' +
					event +
					'" used.'
			);
	}
