...
function push_heap_path(heap, index) {
	...
	for (
		i = (h - 1) >> 1;
		h > 0 && _cost[index] < _cost[heap[i + 1]];
		i = (h - 1) >> 1
	) {
		heap[h + 1] = heap[i + 1];
		h = i;
...
function update_heap_path(heap, index) {
	...
	if (h === heap[0]) {
		throw new Error(
			"PathFinding::update_heap_path() - Error updating head path"
		);
	}
...
function searchLong(x0, y0, x1, y1, range, out) {
	...
	while (i <= MAX_WALKPATH) {
		...
		if (
			!result.success &&
			GAT.cells[x + y * GAT.width] === GAT.type.SNIPABLE
		) {
...
function search(x0, y0, x1, y1, range, out) {
	...
	while (true) {
		...
		if (
			(dirFlag & (2 + 1)) === 2 + 1 &&
			types[x - 1 + (y + 1) * width] & TYPE.WALKABLE
		) {
			error += add_path(
				heap,
				x - 1,
	...
	// Reorganize Path
	for (
		pathLen = 0, i = currentNode;
		pathLen < 100 && i !== calc_index(x0, y0);
		i = _before[i], pathLen++
	);
