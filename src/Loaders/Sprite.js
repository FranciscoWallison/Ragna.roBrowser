...
SPR.prototype.load = function load(data) {
	...
	if (this.header != "SP") {
		throw new Error(
			'SPR::load() - Incorrect header "' + this.header + '", must be "SP"'
		);
	}
...
SPR.prototype.compile = function compile() {
	...
	for (i = 0; i < count; ++i) {
		...
		// RGBA Images
			...
			for (y = 0; y < height; ++y) {
				for (x = 0; x < width; ++x) {
					out[((y + start_y) * gl_width + (x + start_x)) * 4 + 0] =
						data[((height - y - 1) * width + x) * 4 + 3];
					out[((y + start_y) * gl_width + (x + start_x)) * 4 + 1] =
						data[((height - y - 1) * width + x) * 4 + 2];
					out[((y + start_y) * gl_width + (x + start_x)) * 4 + 2] =
						data[((height - y - 1) * width + x) * 4 + 1];
					out[((y + start_y) * gl_width + (x + start_x)) * 4 + 3] =
